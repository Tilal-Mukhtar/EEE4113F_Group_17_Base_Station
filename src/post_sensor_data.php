    ...
    if ($api_key == $api_key_value) {
        ...
        if (!$db) {
            ...
            for ($i = 0; $i < count($_POST["date"]); $i++) {
                $query = "INSERT INTO sensor_data (date, time, temperature, humidity, battery_temperature, battery_humidity, battery_voltage, battery_current)
                VALUES ('" . $_POST["date"][$i] . "', '" . $_POST["time"][$i] . "', '" . $_POST["temperature"][$i] . "', '" . $_POST["humidity"][$i] . "',
                '" . $_POST["battery_temperature"][$i] . "', '" . $_POST["battery_humidity"][$i] . "', '" . $_POST["battery_voltage"][$i] . "', '" . $_POST["battery_current"][$i] . "')";
